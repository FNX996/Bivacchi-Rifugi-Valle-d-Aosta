...
<div style="font-size: 13px; color: #555; background-color: #f8f9fa; padding: 10px; border-radius: 5px; border-left: 4px solid #333; margin-bottom: 15px;">
    <b>App Rifugi & Bivacchi VdA</b><br>Versione: 9.1<br>Autore: Nori Fabrizio
</div>
""", unsafe_allow_html=True)
...
with st.sidebar.expander("🆕 Changelog & Novità", expanded=False):
    ...
    * 🛆 **Nuovi Marker:** Introdotto un design specifico a triangolo per le vette. Colore Blu per i Tremila e Nero per i Quattromila.
...
with tab_gpx:
    ...
    st.markdown("Carica i tuoi file GPX per comprimerli e salvarli permanentemente. Puoi anche **Inviarli alla Mappa** per agganciarli ai sentieri ufficiali ed esplorarli in **3D**.")
            ...
            for t in tracce_feed:
                ...
                with st.container(border=True):
                    ...
                    if dati.get("quote"):
                        ...
                        # Mappa 2D fallback se mancano quote
...
with tab_mappa:
    with st.container(border=True):
        ...
        # Area elimina tappe se presenti
    ...
    if mostra_cime and not mappa_cime.empty:
        for _, r in mappa_cime.iterrows():
            ...
            <div style='position: relative; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center;'>
                <svg width="24" height="24" viewBox="0 0 24 24" style="filter: drop-shadow(1px 1px 2px rgba(0,0,0,0.5));">
                    <polygon points="12,2 22,20 2,20" fill="{colore_cima}" stroke="white" stroke-width="1.5"/>
                </svg>
                <div style='position: absolute; bottom: 0px; right: 0px; width: 10px; height: 10px; background-color: {stato_col}; border-radius: 50%; border: 1.5px solid white; box-shadow: 0 0 2px rgba(0,0,0,0.3);'></div>
            </div>
            """
    ...
    <div style="position: fixed; bottom: 30px; left: 30px; z-index: 99999; pointer-events: auto;">
        ...
        <div id="legenda-mappa-vda" style="display: none; margin-top: 10px; width: 240px; background-color: rgba(255, 255, 255, 0.95); padding: 15px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.3); font-family: sans-serif; font-size: 12px; border: 1px solid #ccc; color: #333;">
            ...
            <div style="margin-bottom: 8px;">
                ...
                <div style="display: flex; align-items: center; margin-top: 4px;"><svg width="16" height="16" style="margin-right: 8px;"><polygon points="8,2 15,14 1,14" fill="#0055ff" stroke="white" stroke-width="1"/></svg><span style="color: #333;">Vetta 3000-3999m</span></div>
                <div style="display: flex; align-items: center; margin-top: 4px;"><svg width="16" height="16" style="margin-right: 8px;"><polygon points="8,2 15,14 1,14" fill="black" stroke="white" stroke-width="1"/></svg><span style="color: #333;">Vetta >4000m</span></div>
            </div>
            <div>
    ...
    st.markdown("---")
    st.markdown("#### 🗺️ Opzioni Mappa & Esplorazione 3D")
    ...
    with col_opt2:
        ...
        if tracce_3d:
            ...
            st.info("💡 Calcola un itinerario o carica un GPX per attivare il 3D.")
